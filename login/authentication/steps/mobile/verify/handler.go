...
func New[acc account.Model](logger log.Logger,
	...
	base *base.Handler[acc], otp otp.Handler,
) handler.Handler[acc] {
	h := &_handler[acc]{
		Handler: base,
		logger:  logger,
...
func (h *_handler[acc]) WithConfig(
	registry registry.Registry) handler.Handler[acc] {
	if err := registry.Unmarshal(&h.config); err != nil {
		panic(err)
	}
	return h
